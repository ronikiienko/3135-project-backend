...
if ($shelter['assigned_admin_id'] !== null) {
    error_response('ALREADY_ASSIGNED', 409);
}

$stmt = $db->prepare('UPDATE shelters SET assigned_admin_id = :admin_id WHERE id = :id');
$stmt->execute([':admin_id' => $session['user_id'], ':id' => $shelterId]);
